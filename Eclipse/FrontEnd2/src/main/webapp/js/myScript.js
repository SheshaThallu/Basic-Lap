...
// UI URL
const UI_URL = "http://localhost:8080/FrontEnd2";
...
async function checkLoginAPI(loginObj) {
	...
	if (customer) {
		...
		localStorage.setItem("FIRST_NAME", customer.firstName);
		localStorage.setItem("LAST_NAME", customer.lastName);
...
{
	const url = BASE_URL + '/customer/' + email;
	const data = {
		method: 'GET',
		headers: {
			'Accept': 'application/json',
...
function fetchAccountNum() {
	const accNo = document.getElementById('accountNum');
